...
class TierCtrl( wx.Window ):
    ...
    def MoveWindow(self, pos, size):
        ...
        # New width
        if ow != w:
            logging.debug('Set Size')
            self.SetSize( wx.Size(w,oh) )
            torepaint = True

        # New height
        if oh != h:
            logging.debug('Set height')
            self.SetHeight(h)
            torepaint = True
    ...
    def AutoAdjustFont(self):
        ...
        logging.debug(' AutoAdjustFont. Enter (h=%d)'%h)
        ...
        logging.debug(' AutoAdjustFont. Leave (h=%d)'%h)
    ...
    def OnPaint(self, event):
        ...
        logging.debug('Recieved an event paint')
        self.Draw(dc)
    ...
    def DrawPane(self, dc, x,y, w,h):
        ...
        # Write the tier name
        textwidth, textheight = dc.GetTextExtent( self._tier.Name )
        # Vertical position
        y = (h - textheight)/2
        # Write text
        dc.SetBackgroundMode( wx.TRANSPARENT )
        ...
        dc.DrawText(self._tier.Name, x+1, y)
    ...
    def _drawAnnotation(self, ann):
        ...
        th=th-2
        (tx,ty) = self.GetPosition()
        ty=ty+1

        # Draw the label
        xpt1, wpt1 = self._calcPointXW(point.GetValue())
        logging.debug(' ... draw annotation: x=%d,w=%d'%(xpt1,wpt1))
        if self._tier.IsPoint():
            label.MoveWindow(wx.Point(tx+xpt1+wpt1,ty), wx.Size(50,th))
            #logging.debug(' ... ... draw label pos: x=%d,y=%d  size=%d-%d'%(tx+xpt1+wpt1,ty,50,th))

        else:
            xpt2, wpt2 = self._calcPointXW(point2.GetValue())
            #logging.debug(' ... ... draw label pos: x=%d,y=%d  size=%d-%d'%(tx+xpt1+wpt1,ty,xpt2-xpt1-wpt1,th))
        ...
        if self._tier.IsInterval():
            ...
            #logging.debug(' ... ... draw point pos: x=%d,y=%d  height=%d'%(tx,ty,th))
    ...
    def _addAnnotationInterval(self, ann):
        ...
        label = LabelCtrl(self, id=-1, label=ann.Text)
    ...
    def _calcPointXW(self, point):
        ...
        b = point.GetValue() - point.GetRadius()
        e = point.GetValue() + point.GetRadius()
        # hum.... take care:
        # b can be "before" tiermintime
        # e can be "after" tiermaxtime

        #logging.debug(' ------- %d,%d  %f,%f  %f  / %f,%f'%(tierwidth,tierheight,tiermintime,tiermaxtime,tierduration,b,e))
