...
# User-Interface
MSG_HEADER="SPPAS test_annotations.sh, a program written by Brigitte Bigi."
TODAY=$(date "+%Y-%m-%d")
...
function normalize {
    ...
    # no longer valid: now we print one token at a line
...
function syllabify {
    ...
    if [ -e DGtdA05Np1_95-salign.TextGrid ]; then
        size=`cat DGtdA05Np1_95-salign.TextGrid | grep -c "size = 2"`
    ...
    if [ -e DGtdA05Np1_95-salign.TextGrid ]; then
        size=`cat DGtdA05Np1_95-salign.TextGrid | grep -c "size = 4"`
        rm DGtdA05Np1_95-salign.TextGrid
        if [ "$size" == "1" ]; then
            fct_echo_status 0
        else
